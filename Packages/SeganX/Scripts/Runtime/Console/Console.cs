    ...
    public class Console : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            foreach (var assembly in assemblies)
            {
                ...
                var allMethods = assembly.GetTypes().SelectMany(x => x.GetMethods()).Where(y => y.GetCustomAttribute<ConsoleAttribute>() != null).ToList();
                foreach (var method in allMethods)
                {
                    var attrib = method.GetCustomAttribute<ConsoleAttribute>();
                    if (attrib.cmdName.IsNullOrEmpty()) attrib.cmdName = method.Name.ToLower();
                    if (attrib.cmdhelp.IsNullOrEmpty()) attrib.cmdhelp = GenerateMethodHelp(method);
                    methods.Add(new MethodObject() { space = attrib.cmdSpace, name = attrib.cmdName, help = attrib.cmdhelp, info = method });
                }
            }
